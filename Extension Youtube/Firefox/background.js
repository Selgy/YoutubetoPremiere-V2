...
// Debug: Version check
console.log('YTP: Background script loaded - Version 3.0.12 with Enhanced Cookie Extraction (Firefox)');

// Firefox compatibility layer
const extensionAPI = typeof browser !== 'undefined' ? browser : chrome;

// Handle browser action click (Firefox uses browserAction instead of action)
if (extensionAPI.browserAction) {
    extensionAPI.browserAction.onClicked.addListener((tab) => {
        console.log("Extension icon clicked!");
    });
} else if (extensionAPI.action) {
...
async function getYouTubeCookies() {
    try {
        ...
        for (const domain of domains) {
            try {
                const domainCookies = await extensionAPI.cookies.getAll({ domain });
        ...
        try {
            ...
            if (tabs[0] && tabs[0].url && tabs[0].url.includes('youtube.com')) {
                const tabCookies = await extensionAPI.cookies.getAll({ url: tabs[0].url });
